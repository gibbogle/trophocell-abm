...
read(nfcell,*) NTcells0						! initial number of cells
read(nfcell,*) Radius0						! radius of region of initial placement of cells (grids)
read(nfcell,*) ntgui						! interval between GUI outputs (timesteps)
read(nfcell,*) idelay						! simulation step delay (ms)
read(nfcell,*) iwrap						! wrapping switch
read(nfcell,*) ichemo_1						! switch for chemokine #1
read(nfcell,*) grad_amp(1)					! chemokine #1 gradient amplitude
read(nfcell,*) grad_dir(1)					! chemokine #1 gradient direction (deg)
...
if (.not.SIMULATE_2D) then
	call logger('Error: only SIMULATE_2D is currently simulated')
	return
endif
...
ntagged  = 0
kcell = 0
! Place NTcells0 cells randomly within a disk of radius = Radius0 centred on (x0,y0,z0) (units = lattice grids)
xmin = x0 - Radius0 - 1
xmax = x0 + Radius0 + 1
ymin = y0 - Radius0 - 1
ymax = y0 + Radius0 + 1
z = z0
do
	x = random_int(xmin,xmax,kpar)
	y = random_int(ymin,ymax,kpar)
